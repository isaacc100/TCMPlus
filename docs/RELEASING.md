...
The Windows package ID is `TCMPlusDesktop`, intentionally distinct from the existing `%LocalAppData%\TCMPlus` session-data root. Do not change it to `TCMPlus`: Velopack owns and may remove its complete package directory during uninstall.
