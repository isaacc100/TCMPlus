...
public partial class TerminalConnectView : UserControl
{
    ...
    public void Deactivate() => CancelOperation();
    ...
    private async Task RefreshHostsAsync()
    {
        ...
        try
        {
            ReplaceHosts(await TerminalDiscoveryClient.DiscoverAsync(cancellationToken: _operationCancellation.Token));
        ...
        catch (Exception exception) { DiscoveryMessage.Text = $"Automatic discovery was unavailable: {exception.Message}"; }
    ...
    private async void OnFindHost(object? sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ReplaceHosts(await TerminalDiscoveryClient.ResolveAsync(
        ...
        catch (Exception exception) { ValidationMessage.Text = exception.Message; }
        finally { SetControlsEnabled(true); }
    }
    ...
    private async void OnConnect(object? sender, RoutedEventArgs e)
    {
        ...
        if (HostList.SelectedItem is not TerminalDiscoveredHost host)
        {
        ...
        try
        {
            var clientVersion = typeof(TerminalConnectView).Assembly
                .GetCustomAttribute<AssemblyInformationalVersionAttribute>()?.InformationalVersion
                ?? typeof(TerminalConnectView).Assembly.GetName().Version?.ToString()
                ?? "unknown";
            _pairingSession = await TerminalPairingClient.StartAsync(host.Host, terminalName, clientVersion, _operationCancellation.Token);
            ...
            {
                throw new InvalidOperationException("The host identity changed during pairing. Refresh the host list and try again.");
            }
            await _preferencesStore.SaveAsync(new TerminalConnectionPreferences(terminalName, host.Address), _operationCancellation.Token);
            ValidationMessage.Text = "Approved. Connecting securely…";
            ConnectionRequested?.Invoke(this, new TerminalConnectionDraft(
                result.HostInstanceId, result.Host, result.TerminalName, result.Password, result.CertificateFingerprint));
        }
        catch (OperationCanceledException) { }
